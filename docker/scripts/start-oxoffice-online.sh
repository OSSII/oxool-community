...
mv certs/servers/localhost/privkey.pem /etc/oxool/key.pem
mv certs/servers/localhost/cert.pem /etc/oxool/cert.pem
mv certs/ca/root.crt.pem /etc/oxool/ca-chain.cert.pem
fi
...
if test -n "${aliasgroup1}" -o -n "${domain}" -o -n "${remoteconfigurl}"; then
    perl -w /start-oxoffice-online.pl || { exit 1; }
fi
