...
# start-oxoffice-online.sh will be moved here; for the moment
...
# Update the /etc/oxool/oxoolwsd.xml according to the env. variables from the YAML file
sub rewrite_config($) {
    my ($config) = @_;
    my $output = '';
...
rewrite_config('/etc/oxool/oxoolwsd.xml');
